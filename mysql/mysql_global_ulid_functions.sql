-- ========================================
-- Global ULID Functions for MySQL
...
-- Function to generate ULID for specific timestamp (partial implementation)
-- Note: This is a simplified version. Full random generation requires external tools.
...
DETERMINISTIC
SQL SECURITY INVOKER
COMMENT 'Generate ULID string for given timestamp (simplified - use C++ for full implementation)'
BEGIN
    DECLARE encoding CHAR(32) DEFAULT '0123456789ABCDEFGHJKMNPQRSTVWXYZ';
    DECLARE result CHAR(26) DEFAULT '';
    ...
    -- Add simplified random part (not cryptographically secure)
    -- In production, use C++ ULID class for proper random generation
    SET random_part = '0000000000000000';
    SET result = CONCAT(LEFT(result, 10), random_part);
...
SELECT 'Global ULID functions created in mysql database:' as info;
SELECT ROUTINE_NAME, ROUTINE_TYPE, ROUTINE_COMMENT
FROM INFORMATION_SCHEMA.ROUTINES
WHERE ROUTINE_SCHEMA = 'mysql'
  AND ROUTINE_NAME LIKE 'ULID%'
  OR ROUTINE_NAME LIKE '%ULID';

-- Usage examples (can be run from any schema):
SELECT 'Usage Examples:' as info;
SELECT '-- Convert ULID string to binary:' as example;
SELECT 'SELECT mysql.ULID_TO_BIN("01FJYWZ3RJM927XKDJGDR06REA");' as query;

SELECT '-- Convert binary back to ULID string:' as example;
SELECT 'SELECT mysql.BIN_TO_ULID(mysql.ULID_TO_BIN("01FJYWZ3RJM927XKDJGDR06REA"));' as query;

SELECT '-- Extract timestamp from ULID:' as example;
SELECT 'SELECT mysql.ULID_TIMESTAMP("01FJYWZ3RJM927XKDJGDR06REA");' as query;

SELECT '-- Convert ULID to DATETIME:' as example;
SELECT 'SELECT mysql.ULID_TO_DATETIME("01FJYWZ3RJM927XKDJGDR06REA");' as query;
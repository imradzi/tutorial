...
std::vector<PostcodeEntry> readPostcodes(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        if (pipePos == std::string::npos) {
            std::cerr << "Warning: Invalid format at line " << lineNumber << std::endl;
            continue;
        }
        
        std::string csvData = line.substr(pipePos + 1);
        std::vector<std::string> fields = parseCSVLine(csvData);
        
        if (fields.size() >= 4) {
            postcodes.emplace_back(fields[0], fields[1], fields[2], fields[3]);
        } else {
            std::cerr << "Warning: Insufficient fields at line " << lineNumber << std::endl;
        }
    }
...
int main() {
    ...
    std::cout << "\n=== Postcodes for Perlis (PLS) ===" << std::endl;
    int count = 0;
    ...
    std::cout << "Enter state code to search (e.g., PLS, JHR): ";
    std::cin >> searchCode;
    ...
    for (const auto& postcode : postcodes) {
        if (postcode.stateCode == searchCode && count < 20) {  // Limit to 20 results
            std::cout << postcode.area << " (" << postcode.postcode << ")" << std::endl;
            count++;
        }
    }
    ...
    if (count == 0) {
        ...
        std::cout << "Showing " << count << " results." << std::endl;
    }

...
namespace PDF {
  ...
  void error_handler(HPDF_STATUS e, HPDF_STATUS d, void* user_data) {
    auto err = static_cast<PDF::Error*>(user_data);
    err->error_no = e;
    err->detail_no = d;
    err->error_message = fmt::format("{} -> ERROR: error_no={:04X}, detail_no={:04X}, message={}", err->current_function, (int64_t)e, (int64_t)d, PDF::GetErrorMessage(e) + " " + PDF::GetErrorMessage(d));
